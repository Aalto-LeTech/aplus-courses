...
class PluginSettings internal constructor(private val applicationPropertiesManager: PropertiesManager) :
    ...
    private val propertyGraph: PropertyGraph = PropertyGraph("A+ Courses")

    val exercicesSearchProperty = propertyGraph.property("")
...
//            courseProject.readAuthenticationFromStorage(passwordStorage, authenticationFactory)
            ...
            mainViewModel.toolWindowCardViewModel.setModuleButtonRequiresLogin(
                courseProject.course.requiresLoginForModules()
            )
...
//            courseProject.exercisesUpdater.restart()
    ...
    fun getCourseFileManager(project: Project): CourseFileManager {
        return courseFileManagers.computeIfAbsent(
            ProjectKey(project)
        ) { key: ProjectKey? -> CourseFileManager(project, DefaultNotifier()) }
    }
    ...
    var importedIdeSettingsId: String?
        get() = applicationPropertiesManager.getValue(LocalIdeSettingsNames.A_PLUS_IMPORTED_IDE_SETTINGS.id)
        set(courseId) {
            applicationPropertiesManager.setValue(LocalIdeSettingsNames.A_PLUS_IMPORTED_IDE_SETTINGS.id, courseId)
        }
...
//            get() = InstanceHolder.INSTANCE
    ...
    companion object {
        ...
        const val MODULE_REPL_INITIAL_COMMANDS_FILE_NAME
                : String = ".repl-commands"
