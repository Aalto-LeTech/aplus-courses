...
object ModuleUtils:
  ...
  val Logger: Logger = LoggerFactory.getLogger(ModuleUtils.getClass)
  ...
  private def clearCommands(@NotNull imports: Array[String]): Array[String] =
    imports.clone
      .map(_.replace("import ", ""))
      .map(_.replace(".*", ""))

  private def getCommandsText(@NotNull imports: Array[String]): String =
  ...
  def getUpdatedText(
    ...
    val commonText = getAndReplaceText(
  ...
  private def getPrettyKeyMapString(@NotNull actionId: String): String =
    ...
    if shortCuts.nonEmpty then
      shortCuts.head.toString
        ...
        .map(_.capitalize)
        .mkString("+")
    else getText("ui.repl.console.welcome.shortcutMissing")
